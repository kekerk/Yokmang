...
public class ControllerMethodServlet extends HttpServlet {
    ...
    public void init(ServletConfig config) throws ServletException{
    	...
    	try {
    		...
    		// properties 파일의 " /model2/hello.bo=hello " 에서  key : /model2/hello.bo  value:hello
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			// 				  설정된 이름에 해당하는 메서드 객체.
			//                메서드 자체를 객체화.
			Method method = BoardAllAction.class.getMethod(methodName, paramType);  //오버로딩 : 매개변수 타입이 다르면 다른 메서드를 찾게 된다.
			// invoke : method를 호출.
