...
<%-- empty : b.file1==null || b.file1=='' --%>
	...
	<c:if test="${pageNum >1 }"><a href="list.bo?pageNum=${pageNum-1}">[이전]</a></c:if>
	<c:forEach var="a" begin="${startpage}" end="${endpage}" >
		<c:if test="${a==pageNum}">[${a}]</c:if>
		<c:if test="${a!=pageNum }"><a href="list.bo?pageNum=${a}">[${a}]</a></c:if>
	</c:forEach>
	<c:if test="${pageNum >= maxpage }">[다음]&nbsp;</c:if>
	<c:if test="${pageNum < maxpage }"><a href="list.bo?pageNum=${pageNum+1}">[다음]</a></c:if>
	</td>
	</tr>
	</c:if>
