...
<table border="1" style="border-collapse: collapse;" class="w3-table-all w3-hoverable">
	...
	<c:if test="${pageNum >1 }"><a href="searchlist.bo?pageNum=${pageNum-1}">[이전]</a></c:if>
	<c:forEach var="a" begin="${startpage}" end="${endpage}" >
		<c:if test="${a==pageNum}">[${a}]</c:if>
		<c:if test="${a!=pageNum }"><a href="searchlist.bo?pageNum=${a}">[${a}]</a></c:if>
	</c:forEach>
	<c:if test="${pageNum >= maxpage }">[다음]&nbsp;</c:if>
	<c:if test="${pageNum < maxpage }"><a href="searchlist.bo?pageNum=${pageNum+1}">[다음]</a></c:if>
	</td>
	</tr>
	</c:if>
